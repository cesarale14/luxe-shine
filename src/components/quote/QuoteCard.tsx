 ...
 *
 * My Home: address, size, mobile. My Rental: zip, units, booking platform, mobile.
 * (Address is a plain required field — no autocomplete; the zip is parsed out of it for
 * the payload + /quote prefill when present.)
 *
...
export function QuoteCard({ sourcePage = "home" }: { sourcePage?: string }) {
  ...
  const [values, setValues] = useState({ address: "", zip: "", size: "", units: "", platform: "", phone: "", company: "" });
  ...
  async function handleSubmit(evt: FormEvent) {
    ...
    const zip = mode === "home" ? zipFromAddress(values.address) : values.zip;
